...
package test.copy_of_service_dwh_0_1;
...
/**
 * Job: Copy_of_service_dwh Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 8.0.1.20211109_1610
 * @status 
 */
public class Copy_of_service_dwh implements TalendJob {
...
protected static void logIgnoredError(String message, Throwable cause) {
	...
	private final String jobName = "Copy_of_service_dwh";
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				Copy_of_service_dwh.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(Copy_of_service_dwh.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
    final static byte[] commonByteArrayLock_TEST_Copy_of_service_dwh = new byte[0];
    static byte[] commonByteArray_TEST_Copy_of_service_dwh = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_TEST_Copy_of_service_dwh.length) {
				if(length < 1024 && commonByteArray_TEST_Copy_of_service_dwh.length == 0) {
   					commonByteArray_TEST_Copy_of_service_dwh = new byte[1024];
				} else {
   					commonByteArray_TEST_Copy_of_service_dwh = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_TEST_Copy_of_service_dwh, 0, length);
			strReturn = new String(commonByteArray_TEST_Copy_of_service_dwh, 0, length, utf8Charset);
		}
		return strReturn;
	}
	...
	private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException{
		...
		if (length == -1) {
			...
			unmarshaller.readFully(commonByteArray_TEST_Copy_of_service_dwh, 0, length);
			strReturn = new String(commonByteArray_TEST_Copy_of_service_dwh, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_TEST_Copy_of_service_dwh) {
...
public int hashCode() {
if(hashCodeDirty) {
int prime = 31;hashCode = prime * hashCode + (id_service == null ? 0 : id_service.hashCode());
hashCodeDirty = false;
}
return hashCode;
}
...
String dbUser_tDBSCD_1 = null;
	...
	final String decryptedPassword_tDBSCD_1 = routines.system.PasswordEncryptUtil.decryptPassword("enc:routine.encryption.key.v1:3m0pgUMinm/aWpngeD6+D7AlTdmwRH1dPQ315Ipc");
    ...
    if(dbschema_tDBSCD_1 == null || dbschema_tDBSCD_1.trim().length() == 0) {
        tableName_tDBSCD_1 = "Dim_Service01";
    } else {
        tableName_tDBSCD_1 = dbschema_tDBSCD_1 + "].[" + "Dim_Service01";
    }
    ...
    String tmpValue_tDBSCD_1 = null;    
        String search_tDBSCD_1 = "SELECT [id_service], [description], [title], [price] FROM [" + tableName_tDBSCD_1 + "] WHERE [scd_end] IS NULL";
        ...
        while(resultSet_tDBSCD_1.next()) {
            ...
            SCDStruct_tDBSCD_1 row_tDBSCD_1 = new SCDStruct_tDBSCD_1();
                    if(resultSet_tDBSCD_1.getObject(1) != null) {
                        sk_tDBSCD_1.id_service = resultSet_tDBSCD_1.getInt(1);
                    }
                    ...
                    if(resultSet_tDBSCD_1.getObject(3) != null) {
                        row_tDBSCD_1.title = resultSet_tDBSCD_1.getString(3);
                    }
                    if(resultSet_tDBSCD_1.getObject(4) != null) {
                        row_tDBSCD_1.price = resultSet_tDBSCD_1.getFloat(4);
                    }
            cache_tDBSCD_1.put(sk_tDBSCD_1, row_tDBSCD_1);
        }
        resultSet_tDBSCD_1.close();
        statement_tDBSCD_1.close();
    String insertionSQL_tDBSCD_1 = "INSERT INTO [" + tableName_tDBSCD_1 + "]([id_service], [description], [title], [price], [scd_start], [scd_end]) VALUES(?, ?, ?, ?, ?, ?)";
    java.sql.PreparedStatement insertionStatement_tDBSCD_1 = conn_tDBSCD_1.prepareStatement(insertionSQL_tDBSCD_1);
            insertionStatement_tDBSCD_1.setTimestamp(5, new java.sql.Timestamp(start_Hash.get("tDBSCD_1")));
            insertionStatement_tDBSCD_1.setNull(6, java.sql.Types.DATE);
        String updateSQLForType1_tDBSCD_1 = "UPDATE [" + tableName_tDBSCD_1 + "] SET [description] = ?, [title] = ? WHERE [id_service] = ?";
        java.sql.PreparedStatement updateForType1_tDBSCD_1 = conn_tDBSCD_1.prepareStatement(updateSQLForType1_tDBSCD_1);        
        String updateSQLForType2_tDBSCD_1 = "UPDATE [" + tableName_tDBSCD_1 + "] SET [scd_end] = ? WHERE [id_service] = ? AND [scd_end] IS NULL";
 ...
 */
	...
	final String decryptedPassword_tDBInput_1 = routines.system.PasswordEncryptUtil.decryptPassword("enc:routine.encryption.key.v1:6vJZN5uqhIeHW1I4BeOe3rFB8HZ+OJId7KjnDwH4BA==");
...
+" PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY price) OVER ()\n    ) AS price\nFROM  Service ;";
            ...
            row1.id_service = rs_tDBInput_1.getInt(1);
            ...
            row1.price = rs_tDBInput_1.getFloat(4);
            if(rs_tDBInput_1.wasNull()){
                    row1.price = null;
            }
		                    }
 ...
 */
	...
	try {
        lookUpKey_tDBSCD_1 = new SCDSK_tDBSCD_1();
            lookUpKey_tDBSCD_1.id_service = row1.id_service;
...
insertionStatement_tDBSCD_1.setInt(1, row1.id_service);
}

                    if(row1.description == null) {
insertionStatement_tDBSCD_1.setNull(2, java.sql.Types.VARCHAR);
} else {
insertionStatement_tDBSCD_1.setString(2, row1.description);
}

                    if(row1.title == null) {
insertionStatement_tDBSCD_1.setNull(3, java.sql.Types.VARCHAR);
} else {
insertionStatement_tDBSCD_1.setString(3, row1.title);
}

                    if(row1.price == null) {
insertionStatement_tDBSCD_1.setNull(4, java.sql.Types.FLOAT);
} else {
insertionStatement_tDBSCD_1.setFloat(4, row1.price);
}

        nb_line_inserted_tDBSCD_1 += insertionStatement_tDBSCD_1.executeUpdate();
    } else {
            if((lookUpValue_tDBSCD_1.description == null && row1.description!= null) || (lookUpValue_tDBSCD_1.description != null && !lookUpValue_tDBSCD_1.description.equals(row1.description)) || (lookUpValue_tDBSCD_1.title == null && row1.title!= null) || (lookUpValue_tDBSCD_1.title != null && !lookUpValue_tDBSCD_1.title.equals(row1.title))) {
                    if(row1.description == null) {
updateForType1_tDBSCD_1.setNull(1, java.sql.Types.VARCHAR);
} else {
updateForType1_tDBSCD_1.setString(1, row1.description);
}

                    if(row1.title == null) {
updateForType1_tDBSCD_1.setNull(2, java.sql.Types.VARCHAR);
} else {
updateForType1_tDBSCD_1.setString(2, row1.title);
}

                    if(row1.id_service == null) {
updateForType1_tDBSCD_1.setNull(3, java.sql.Types.INTEGER);
} else {
updateForType1_tDBSCD_1.setInt(3, row1.id_service);
}

                nb_line_update_tDBSCD_1 += updateForType1_tDBSCD_1.executeUpdate();
            }
            if((lookUpValue_tDBSCD_1.price == null && row1.price!= null) || (lookUpValue_tDBSCD_1.price != null && !lookUpValue_tDBSCD_1.price.equals(row1.price))) {
                    if(row1.id_service == null) {
updateForType2_tDBSCD_1.setNull(2, java.sql.Types.INTEGER);
} else {
updateForType2_tDBSCD_1.setInt(2, row1.id_service);
}
...
globalMap.put("tDBSCD_1_ERROR_MESSAGE",e.getMessage());
                ...
                lookUpValue_tDBSCD_1.description = row1.description;
                lookUpValue_tDBSCD_1.title = row1.title;
                lookUpValue_tDBSCD_1.price = row1.price;
 ...
 */
    ...
    public static void main(String[] args){
        final Copy_of_service_dwh Copy_of_service_dwhClass = new Copy_of_service_dwh();

        int exitCode = Copy_of_service_dwhClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = Copy_of_service_dwh.class.getClassLoader().getResourceAsStream("test/copy_of_service_dwh_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = Copy_of_service_dwh.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
                try {
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Copy_of_service_dwh");
        }
...
/************************************************************************************************
 *     50676 characters generated by Talend Open Studio for Data Integration 
 *     on the 29 mars 2026, 15:08:28 WAT
 ************************************************************************************************/